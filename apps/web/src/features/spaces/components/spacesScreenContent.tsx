...
export const SpacesScreenContent = () => {
	const { data: spaces } = useSuspenseQuery(spacesQuery);
	...
	const otherNames = (spaceId: string) =>
		spaces.filter((space) => space.id !== spaceId).map((space) => space.name);

	const active = spaces.filter((space) => !space.archivedAt);
	const archived = spaces.filter((space) => space.archivedAt);
			...
			{creating ? (
				<div className="rounded-xl border border-border p-3">
					<NameForm
						...
						takenNames={spaces.map((space) => space.name)}
